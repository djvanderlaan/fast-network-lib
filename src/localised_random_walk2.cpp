...
class localised_random_walk2_computer {
  public:
    localised_random_walk2_computer(const Graph& graph_, 
    ...
    bool computation_step(unsigned int nthreads) {
      ...
      for (unsigned int i = 0; i < nthreads; ++i) {
        futures.emplace_back(
          std::async(std::launch::async, &localised_random_walk2_computer::computation_step_range, 
...
std::vector<double> localised_random_walk2(const Graph& graph, 
  ...
  localised_random_walk2_computer computer(graph, values, weights, 
